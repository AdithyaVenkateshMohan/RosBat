...
def echo_genration(pc_list):
    ...
    Results = numpy.empty((0,), dtype=[('Distance', 'f4'),('Azimuth','f4'),('Elevations','f4')] )
    ...
    for p in pc_list:
        ...
        acc = numpy.array([(p.x, math.degrees(p.y),math.degrees(p.z))], dtype=[('Distance', 'f4'),('Azimuth','f4'),('Elevations','f4')])
        Results= numpy.append(Results, acc)
    print("ready to save")


    if (i==10):
        print("gonna save the array")
        save_numpyarray(Results,name = "samplePoints",FMT=['%f','%f','%f'],HEADER="Distance,Azimuth,Elevations",type = "csv")
